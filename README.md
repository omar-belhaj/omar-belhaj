# 👋 Omar Belhaj

🎓 Master in AI, Data and Agentic - Université Paris-Dauphine (since Sept. 2025)   
🎓 Bachelor in Mathematics & Computer Science - Université Paris-Dauphine (July 2025)   
🤖 Passionate about AI, Machine Learning & Data Science

## 🛠️ Skills

**Programming** : Python, SQL, R, Git  
**Mathematics** : Probability, Bayesian theory, Statistics, Optimization, Monte Carlo Methods   
**ML & Data Science** : scikit-learn, PyTorch, pandas, NumPy, SHAP, LIME, FactoMineR  
**NLP & LLM** : HuggingFace Transformers, RAG, LangChain, LangGraph, Ollama, OpenAI API  
**Cloud** : AWS, Azure, GCP

## 🎯 Interests

Applied AI (Health, Environment, Sports)  
Probabilistic modeling  
Data-driven decision making  

## 📫 Get in touch

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Omar%20Belhaj-0077B5?style=flat&logo=linkedin)](https://linkedin.com/in/omar-belhaj)
[![Email](https://img.shields.io/badge/Email-omar.belhaj%40dauphine.eu-lightgrey?style=flat&logo=gmail)](mailto:omar.belhaj@dauphine.eu)
